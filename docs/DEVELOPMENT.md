...
- 阶段 C 部分完成：真实模型端到端闭环已跑通——隐藏 AI 会话桥接、结构化 JSON 回复提取（含引号容错与角色 ID 映射）、`story_present_choice` 选择卡已绑定为游戏壳内界面（`choice-bridge.ts` + `ChoiceCard.tsx`，经 `/api/respond` 回答，支持刷新恢复重放）。待完成回合按存档持久化为 `dsh-story-ai-pending:<saveId>`，刷新后会从原会话和基线序号恢复；不再因固定 120 秒等待上限丢失长回合结果。
