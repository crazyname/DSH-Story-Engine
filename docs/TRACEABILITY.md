...
| Durable `transactionId` intent、input fingerprint、单向 transaction/hidden-turn journal | `TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Transaction journal 原子持久化、同 transaction 串行、identical replay、collision/fail-closed | `HOST_API.md`、`TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-store.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Host transaction GET/PUT API 与浏览器 persistence primitive | `HOST_API.md` | `client/story-ui/src/index.ts`、`client/story-ui/src/client/host-transactions.ts` | `client/story-ui/test/host-transactions.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
