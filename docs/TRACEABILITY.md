...
| Child `stepKey + operationId` 在 mutating `story_*` body 前持久化；active player transaction 强制 exact `transaction_id`；same-save operation ownership 在最终 journal 写临界区 fail-closed | `TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/core-step-journal.ts`、`transaction-store.ts::findOpenBySession/findOperationOwner/write`、`index.ts` 的 rc.2 `tools/execute` hook | `client/story-ui/test/core-step-journal.test.ts`、`transaction-api.test.ts` | 当前 D2c-1 分支尚未执行 Windows typecheck/test/build；真实 DSH tool dispatch / transaction-bound receipt smoke 待适用验收 | D2c-1 已实现，待验证/合并 |
