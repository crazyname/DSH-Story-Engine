...
| Durable `transactionId` intent、规范化 input/fingerprint、Story UI channel identity、单向 transaction/hidden-turn journal | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Bootstrap 只能写 `prepared` 空证据 intent；新增 hidden turn 从 `planned` 开始且不得伪造 native turn | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts`、`client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-journal.test.ts`、`client/story-ui/test/transaction-store.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Story `turnId` 使用 social-safe ID；`dshRequestId` 独立；`sessionId+dshTurn` 唯一；`planned/uncertain` 不伪造 native turn | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 待 D2b 真实 DSH correlation 验收 | D2a contract 已实现 |
| Hidden dispatch evidence 单向收敛：`uncertain → dispatched/结果态`，已确认 `dispatched` 不倒退到 `uncertain` | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 待 D2b 故障注入验收 | D2a contract 已实现 |
| `activeTurnId` / `canonicalResultTurnId` 引用合法 lifecycle；terminal record standalone 语义非法时 fail-closed | `HOST_API.md`、`TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-journal.ts`、`client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-journal.test.ts`、`client/story-ui/test/transaction-store.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Transaction journal 原子持久化、重启可读、同 transaction 串行、identical replay、collision/fail-closed、fingerprint 重算 | `HOST_API.md`、`TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-store.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Host transaction GET/list/PUT、同源写、400/409、optimistic/collision 冲突 | `HOST_API.md` | `client/story-ui/src/index.ts` | `client/story-ui/test/transaction-api.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Browser transaction bridge load/list/save：path identity、duplicate list 与 exact canonical PUT acknowledgement fail-closed | `HOST_API.md` | `client/story-ui/src/client/host-transactions.ts` | `client/story-ui/test/host-transactions.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
| Hidden identity 分离：Story Engine `turnId`、DSH `dshRequestId/rpcId`、DSH numeric `dshTurn` | `TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-journal.ts`；上游 DSH session contract/history | `client/story-ui/test/transaction-journal.test.ts` | 待 D2b 真实 DSH correlation 验收 | D2a contract 已实现 |
...
- 只有实现和验证证据都存在时，才把一行标记为 `完成`；当前分支新增但尚未跑本地命令的能力写成“已实现，待验证”。
