...
| Durable `transactionId` intent、input fingerprint、Story UI channel identity、单向 transaction/hidden-turn journal | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
...
| Story `turnId`、`dshRequestId`、`sessionId+dshTurn` 唯一性与 canonical-result 引用约束 | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 待 D2b 真实 DSH correlation 验收 | D2a contract 已实现 |
...
| Host transaction GET/list/PUT、同源写、400/409 与浏览器 persistence primitive | `HOST_API.md` | `client/story-ui/src/index.ts`、`client/story-ui/src/client/host-transactions.ts` | `client/story-ui/test/transaction-api.test.ts`、`client/story-ui/test/host-transactions.test.ts` | 尚未执行本地 Windows 验收 | D2a 已实现，待验证 |
