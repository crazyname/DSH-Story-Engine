...
| Durable `transactionId` intent、规范化 input/fingerprint、Story UI channel identity、单向 transaction/hidden-turn journal | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | PR #6 Windows 自动验证通过；未执行真实 DSH/browser recovery | **D2a 完成，待合并** |
| Bootstrap 只能写 `prepared` 空证据 intent；新增 hidden turn 从 `planned` 开始且不得伪造 native turn | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts`、`client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-journal.test.ts`、`client/story-ui/test/transaction-store.test.ts` | PR #6 Windows 自动验证通过 | **D2a 完成，待合并** |
| Story `turnId` 使用 social-safe ID；`dshRequestId` 独立；`sessionId+dshTurn` 唯一；`planned/uncertain` 不伪造 native turn | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 自动 contract 验证通过；真实 DSH correlation 待 D2b | D2a contract 完成 |
| Hidden dispatch evidence 单向收敛：`uncertain → dispatched/结果态`，已确认 `dispatched` 不倒退到 `uncertain` | `TRANSACTION_AND_RECOVERY_SPEC.md`、`HOST_API.md` | `client/story-ui/src/transaction-journal.ts` | `client/story-ui/test/transaction-journal.test.ts` | 自动 lifecycle 验证通过；真实故障注入待 D2b | D2a contract 完成 |
| `activeTurnId` / `canonicalResultTurnId` 引用合法 lifecycle；terminal record standalone 语义非法时 fail-closed | `HOST_API.md`、`TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-journal.ts`、`client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-journal.test.ts`、`client/story-ui/test/transaction-store.test.ts` | PR #6 Windows 自动验证通过 | **D2a 完成，待合并** |
| Transaction journal 原子持久化、重启可读、同 transaction 串行、identical replay、collision/fail-closed、fingerprint 重算 | `HOST_API.md`、`TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-store.ts` | `client/story-ui/test/transaction-store.test.ts` | PR #6 Windows 自动验证通过 | **D2a 完成，待合并** |
| Host transaction GET/list/PUT、同源写、400/409、optimistic/collision 冲突 | `HOST_API.md` | `client/story-ui/src/index.ts` | `client/story-ui/test/transaction-api.test.ts` | PR #6 Windows 自动验证通过；真实浏览器调用待后续集成 | **D2a 完成，待合并** |
| Browser transaction bridge load/list/save：path identity、duplicate list 与 exact canonical PUT acknowledgement fail-closed | `HOST_API.md` | `client/story-ui/src/client/host-transactions.ts` | `client/story-ui/test/host-transactions.test.ts` | PR #6 Windows 自动验证通过；未执行真实浏览器 recovery | **D2a 完成，待合并** |
| Hidden identity 分离：Story Engine `turnId`、DSH `dshRequestId/rpcId`、DSH numeric `dshTurn` | `TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/transaction-journal.ts`；上游 DSH session contract/history | `client/story-ui/test/transaction-journal.test.ts` | contract 自动验证通过；真实 DSH correlation 待 D2b | D2a contract 完成 |
| 玩家 submit/retry/recover 真正接入 journal；hidden dispatch ambiguity reconciliation | `TRANSACTION_AND_RECOVERY_SPEC.md` | 待 D2b coordinator | 待 D2b | 待 D2b crash/restart matrix | 未完成 |
| Host 玩家 projection 保存后、hidden evidence 产生前的 session/bootstrap 或 journal 失败保持可恢复；浏览器 projection 回滚后按同 transaction 恢复且不重复玩家输入 | `TRANSACTION_AND_RECOVERY_SPEC.md` | `client/story-ui/src/client/player-transaction-coordinator.ts` | `client/story-ui/test/player-transaction-pre-dispatch-failure.test.ts` | 自动故障注入覆盖 session bootstrap 与 `beforeDispatch` journal 写入失败；真实浏览器 crash-window 待补 | D2b hotfix 自动验证完成 |
| Child `operationId` step 在 core 调用前持久化并支持部分多-operation recovery | `TRANSACTION_AND_RECOVERY_SPEC.md` | journal primitive 已有；coordinator wiring 待 D2b | 待 D2b | 待 D2b crash/restart matrix | 部分完成 |
| 非终态 transaction 阻止 fork/另存为 | `HOST_API.md`、`TRANSACTION_AND_RECOVERY_SPEC.md` | 待 D2b | 待 D2b | 待 D2b | 未完成 |
