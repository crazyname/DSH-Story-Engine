...
Stage D 已完成 AI canonical social commit 幂等、D1 core operation receipts、D2a durable transaction journal、D2b player transaction coordinator 和 D2c-1 core preflight operation linking；D2c-2 core receipt / tool result → social projection 跨域 reconciliation 已完成实现与合并前验证，PR #13 待合并，下一阶段为 D2d fork / restart / failure matrix。
...
当前版本路线明确区分：v1.0 是基于认证 DSH Runtime 的个人长期游玩版；v1.x 用于 Runtime/Model/Visual/Product abstraction；v2.0 才是默认 Native Runtime、无需安装 DSH 的首个正式对外产品版本。详细边界见 `docs/DEVELOPMENT.md` 和 `docs/NEXT_DEVELOPMENT_PLAN.md`；v1.0 Personal 从当前 Stage D 到 E1–E10 的精确开发顺序、依赖和退出条件见 `docs/PERSONAL_V1_IMPLEMENTATION_PLAN.md`。
