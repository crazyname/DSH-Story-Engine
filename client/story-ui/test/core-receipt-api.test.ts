...
function receipt(operationId='op-a',transactionId='tx-receipt'){return{operationId,transactionId,operation:'story_commit_state',fingerprint:'a'.repeat(64),stateVersion:3,committedAt:'2026-09-03T00:00:00.000Z',result:{ok:true}}}
...
describe('core receipt Host API',()=>{
 it('returns only a receipt owned by the requested save transaction and hidden session',async()=>{
  ...
  const linked=reviseTransaction(planned,{operationRefs:[{stepKey:'step-a',operationId:'op-a'}]});capture=responseCapture();await txRoute.handler(request('PUT','/story-engine/api/transactions/save-a/tx-receipt',{expectedRevision:1,transaction:linked}),capture.res);expect(capture.snapshot().status).toBe(200)
  await writeRuntime(root,'session-a')

  capture=responseCapture();await receiptRoute.handler(request('GET','/story-engine/api/core-receipts/save-a/tx-receipt/op-a'),capture.res);expect(capture.snapshot()).toMatchObject({status:200,body:{sessionId:'session-a',receipt:{operationId:'op-a',transactionId:'tx-receipt'}}})
 ...
 it('fails closed on receipt transaction mismatch and duplicate receipts across hidden sessions',async()=>{
  ...
  const linked=reviseTransaction(planned,{operationRefs:[{stepKey:'step-a',operationId:'op-a'}]});capture=responseCapture();await txRoute.handler(request('PUT','/story-engine/api/transactions/save-a/tx-receipt',{expectedRevision:1,transaction:linked}),capture.res)
