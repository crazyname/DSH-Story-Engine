...
describe('retry recovery matrix',()=>{
 it('updates retry uncertainty diagnostics and recovers the same retry without creating another hidden turn',async()=>{
  ...
  expect(record).toMatchObject({status:'needs-recovery',canonicalResultTurnId:retryTurnId,hiddenTurns:[{turnId:'turn-initial',state:'failed'},{turnId:retryTurnId,kind:'retry',state:'completed',dshTurn:5}]})
