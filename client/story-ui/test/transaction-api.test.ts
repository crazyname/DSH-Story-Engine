...
describe('transaction host API',()=>{
 it('supports PUT, GET, list, optimistic conflict, collision, path errors, and same-origin writes',async()=>{const root=await mkdtemp(join(tmpdir(),'story-tx-api-'));const registrations:any[]=[];const ctx={webServer:{register:(entry:any)=>{registrations.push(entry);return()=>{}}},effect:(factory:()=>unknown)=>factory()} as any;apply(ctx,{runtimeRoot:root,storyRuntimeRoot:join(root,'runtime'),packsRoot:join(root,'packs')});const route=registrations.find(entry=>entry.path==='/story-engine/api/transactions');expect(route).toBeDefined();const first=await createPreparedTransaction({transactionId:'tx:api',saveId:'save-a',channelId:'scene-main',text:'A',baseProjectionRevision:0});
  ...
  const collision=await createPreparedTransaction({transactionId:'tx:api',saveId:'save-a',channelId:'scene-main',text:'B',baseProjectionRevision:0});capture=responseCapture();await route.handler(request('PUT','/story-engine/api/transactions/save-a/tx%3Aapi',{expectedRevision:-1,transaction:collision}),capture.res);expect(capture.snapshot()).toMatchObject({status:409,body:{error:expect.stringContaining('幂等冲突')}})
  const advanced=reviseTransaction(first,{diagnostic:{code:'advanced',message:'first winner'}},new Date('2026-08-30T08:00:01.000Z'));capture=responseCapture();await route.handler(request('PUT','/story-engine/api/transactions/save-a/tx%3Aapi',{expectedRevision:0,transaction:advanced}),capture.res);expect(capture.snapshot()).toMatchObject({status:200,body:{revision:1}})
  const stale=reviseTransaction(first,{diagnostic:{code:'stale',message:'second writer'}},new Date('2026-08-30T08:00:02.000Z'));capture=responseCapture();await route.handler(request('PUT','/story-engine/api/transactions/save-a/tx%3Aapi',{expectedRevision:0,transaction:stale}),capture.res);expect(capture.snapshot()).toMatchObject({status:409,body:{error:expect.stringContaining('版本冲突')}})
  ...
  capture=responseCapture();await route.handler(request('GET','/story-engine/api/transactions/save-a/tx/extra',undefined,undefined),capture.res);expect(capture.snapshot().status).toBe(400)
