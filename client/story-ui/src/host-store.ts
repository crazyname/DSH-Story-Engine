...
export class StoryProjectionStore{
 ...
 async write(id:string,expectedRevision:number,value:JsonObject):Promise<JsonObject>{const current=await this.read(id);const revision=current===undefined?-1:Number(current.revision);if(revision!==expectedRevision)throw new Error(`存档版本冲突：当前 ${revision}，提交基于 ${expectedRevision}`);if(value.saveId!==id||!Number.isInteger(value.revision)||(current!==undefined&&Number(value.revision)!==expectedRevision+1))throw new Error('存档 ID 或新版本无效');const path=this.path(id);await mkdir(dirname(path),{recursive:true});const temporary=`${path}.${process.pid}.tmp`;await writeFile(temporary,`${JSON.stringify(value,null,2)}\n`,'utf8');await rename(temporary,path);return value}
 /** Remove one save. Returns false when the save did not exist. */
 async remove(id:string):Promise<boolean>{try{await rm(this.path(id));return true}catch(error){if((error as NodeJS.ErrnoException).code==='ENOENT')return false;throw error}}
}
