...
export function appendAiMessages(projection:StorySaveProjection,channelId:string,inputs:readonly AiMessageInput[],now=new Date(),commitTurnId?:string):StorySaveProjection{
 ...
 const allowed=new Set([...channel.participantIds,...projection.participants.filter(p=>p.role==='narrator'||p.role==='system').map(p=>p.id)]);const normalized=inputs.map(input=>{if(input.senderId===player?.id)throw new Error('AI 不能替玩家发送消息');if(!allowed.has(input.senderId))throw new Error(`发送者不属于频道：${input.senderId}`);const content=input.content.trim();if(!content)throw new Error('AI 消息内容不能为空');return{senderId:input.senderId,kind:input.kind,content}})
 const stableTurnId=commitTurnId?.trim();if(commitTurnId!==undefined&&!stableTurnId)throw new Error('AI 回合 ID 不能为空')
 if(stableTurnId){const existing=projection.messages.filter(message=>message.turnId===stableTurnId);if(existing.length){const same=existing.length===normalized.length&&existing.every((message,index)=>{const expected=normalized[index]!;return message.channelId===channelId&&message.senderId===expected.senderId&&message.kind===expected.kind&&message.content===expected.content&&message.canonStatus==='committed'});if(same)return projection;throw new Error(`AI 回合提交冲突：${stableTurnId}`)}}
 const createdAt=now.toISOString();const turnId=stableTurnId??`turn-${projection.revision+1}`;const messages=normalized.map((input,index)=>({id:stableTurnId?`ai-${stableTurnId}-${index+1}`:`ai-${now.getTime()}-${projection.messages.length+index+1}`,channelId,senderId:input.senderId,kind:input.kind,content:input.content,createdAt,seasonId:projection.frame.seasonLabel,episodeId:projection.frame.episodeLabel,sceneId:projection.frame.sceneLabel,turnId,canonStatus:'committed' as const}))
