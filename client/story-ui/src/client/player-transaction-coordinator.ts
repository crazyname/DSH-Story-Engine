 ...
 * It persists intent and hidden-attempt evidence before DSH dispatch, but does
 * not claim that DSH rc.2 provides durable prompt request correlation.
 */
export class PlayerTransactionCoordinator{
 ...
 async recover(projection:StorySaveProjection):Promise<RecoveredAiBridgeResult|null>{const record=await this.open(projection.saveId);if(record===undefined)return this.ai.recover(projection);const state:MutableTransaction={record};if(record.hiddenTurns.length===0){if(record.status!=='prepared')throw new Error(`transaction ${record.transactionId} 缺少 hidden evidence，状态为 ${record.status}`);const result=await this.dispatch(state,projection,record.input.channelId,record.input.text,'initial');return{channelId:record.input.channelId,result,turnId:result.turnId!}}
  const recovered=await this.ai.recover(projection);if(recovered===null){const local=this.ai.turn(projection.saveId);const active=state.record.activeTurnId;if(active!==undefined&&!hiddenTerminal(this.findHidden(state.record,active).turn.state))await this.needsRecovery(state,active,new Error(local===null?'本地 pending hidden turn 缺失，rc.2 无 durable request correlation，禁止盲目重发':`hidden turn 未产生可提交结果：${local.state}`),'hidden-recovery-required');return null}if(!state.record.hiddenTurns.some(turn=>turn.turnId===recovered.turnId)){await this.needsRecovery(state,state.record.activeTurnId??state.record.hiddenTurns.at(-1)!.turnId,new Error(`恢复得到未知 hidden turn：${recovered.turnId}`),'hidden-identity-mismatch');throw new Error(`恢复得到的 AI turn 不属于当前 transaction：${recovered.turnId}`)}await this.complete(state,recovered.turnId);return recovered}
 ...
 async acknowledge(saveId:string,turnId:string):Promise<void>{const record=await this.open(saveId);if(record!==undefined&&record.canonicalResultTurnId===turnId){const state:MutableTransaction={record};const{turn}=this.findHidden(record,turnId);if(turn.state!=='completed')throw new Error(`canonical hidden turn 尚未完成：${turnId}`);await this.save(state,{status:'committed',activeTurnId:undefined,diagnostic:undefined})}this.ai.acknowledge(saveId,turnId)}
}
