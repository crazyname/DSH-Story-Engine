...
export function validateTransactionRecord(value:unknown):StoryTransactionRecord{
 ...
 const activeRequestId=raw.activeRequestId===undefined?undefined:stableString(raw.activeRequestId,'activeRequestId');if(activeRequestId!==undefined&&!requestIds.has(activeRequestId))throw new Error('activeRequestId 未引用已知 hidden turn')
 const canonicalResultTurnId=raw.canonicalResultTurnId===undefined?undefined:stableString(raw.canonicalResultTurnId,'canonicalResultTurnId');if(canonicalResultTurnId!==undefined&&!turnIds.has(canonicalResultTurnId))throw new Error('canonicalResultTurnId 未引用已知 hidden turn')
...
export function assertTransactionUpdate(current:StoryTransactionRecord,next:StoryTransactionRecord):void{
 if(next.transactionId!==current.transactionId||next.saveId!==current.saveId)throw new Error('transaction identity 不可修改')
 if(next.inputFingerprint!==current.inputFingerprint||next.input.channelId!==current.input.channelId||next.input.text!==current.input.text||next.baseProjectionRevision!==current.baseProjectionRevision||next.createdAt!==current.createdAt)throw new Error('transaction input identity 不可修改')
 if(TERMINAL_TRANSACTION.has(current.status)&&next.status!==current.status)throw new Error(`终态 transaction 不可改写：${current.status}`)
 if(next.hiddenTurns.length<current.hiddenTurns.length)throw new Error('hidden turn evidence 不可删除')
 for(let index=0;index<current.hiddenTurns.length;index+=1){const before=current.hiddenTurns[index]!;const after=next.hiddenTurns[index]!;if(after.requestId!==before.requestId||after.kind!==before.kind)throw new Error('hidden turn identity 不可修改');if(before.sessionId!==undefined&&after.sessionId!==before.sessionId)throw new Error('hidden session identity 不可修改');if(before.turnId!==undefined&&after.turnId!==before.turnId)throw new Error('hidden turnId 不可修改');if(TERMINAL_TURN.has(before.state)&&after.state!==before.state)throw new Error(`终态 hidden turn 不可改写：${before.requestId}`)}
 if(next.operationRefs.length<current.operationRefs.length)throw new Error('operation identity evidence 不可删除')
 for(let index=0;index<current.operationRefs.length;index+=1){const before=current.operationRefs[index]!;const after=next.operationRefs[index]!;if(after.stepKey!==before.stepKey||after.operationId!==before.operationId)throw new Error('operation identity evidence 不可修改')}
 if(current.canonicalResultTurnId!==undefined&&next.canonicalResultTurnId!==current.canonicalResultTurnId)throw new Error('canonicalResultTurnId 不可修改')
}
