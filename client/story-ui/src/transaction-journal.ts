...
export function validateTransactionRecord(value:unknown):StoryTransactionRecord{
 ...
 const statuses=new Set<StoryTransactionStatus>(['prepared','committed','cancelled','failed','needs-recovery']);if(!statuses.has(raw.status as StoryTransactionStatus))throw new Error('transaction.status 无效');const status=raw.status as StoryTransactionStatus
 if(!Array.isArray(raw.hiddenTurns))throw new Error('hiddenTurns 必须是数组')
 const turnIds=new Set<string>();const requestIds=new Set<string>();const nativeTurns=new Set<string>();const hiddenTurns=raw.hiddenTurns.map((item,index)=>{const entry=object(item,`hiddenTurns[${index}]`);const turnId=storyUiId(entry.turnId,`hiddenTurns[${index}].turnId`);const dshRequestId=entry.dshRequestId===undefined?undefined:stableString(entry.dshRequestId,`hiddenTurns[${index}].dshRequestId`);if(turnIds.has(turnId))throw new Error(`hidden turnId 重复：${turnId}`);turnIds.add(turnId);if(dshRequestId!==undefined){if(requestIds.has(dshRequestId))throw new Error(`hidden dshRequestId 重复：${dshRequestId}`);requestIds.add(dshRequestId)}if(!['initial','retry','continuation'].includes(String(entry.kind)))throw new Error(`hiddenTurns[${index}].kind 无效`);if(!['planned','dispatched','completed','failed','cancelled','uncertain'].includes(String(entry.state)))throw new Error(`hiddenTurns[${index}].state 无效`);const state=entry.state as StoryHiddenTurnState;const sessionId=entry.sessionId===undefined?undefined:stableString(entry.sessionId,`hiddenTurns[${index}].sessionId`);let dshTurn:number|undefined;if(entry.dshTurn!==undefined){if(!Number.isSafeInteger(entry.dshTurn)||Number(entry.dshTurn)<0)throw new Error(`hiddenTurns[${index}].dshTurn 必须是非负安全整数`);if(sessionId===undefined)throw new Error(`hiddenTurns[${index}].dshTurn 需要 sessionId`);if(state==='planned'||state==='uncertain')throw new Error(`hiddenTurns[${index}].${state} 状态不能携带 dshTurn`);dshTurn=Number(entry.dshTurn);const nativeKey=`${sessionId}:${dshTurn}`;if(nativeTurns.has(nativeKey))throw new Error(`DSH native turn 重复：${nativeKey}`);nativeTurns.add(nativeKey)}return{turnId,...(dshRequestId===undefined?{}:{dshRequestId}),kind:entry.kind as StoryHiddenTurnKind,state,...(sessionId===undefined?{}:{sessionId}),...(dshTurn===undefined?{}:{dshTurn})}})
 ...
 const activeTurnId=raw.activeTurnId===undefined?undefined:storyUiId(raw.activeTurnId,'activeTurnId');if(activeTurnId!==undefined){const active=hiddenTurns.find(turn=>turn.turnId===activeTurnId);if(active===undefined)throw new Error('activeTurnId 未引用已知 hidden turn');if(['completed','failed','cancelled'].includes(active.state))throw new Error('activeTurnId 不能引用终态 hidden turn')}
 const canonicalResultTurnId=raw.canonicalResultTurnId===undefined?undefined:storyUiId(raw.canonicalResultTurnId,'canonicalResultTurnId');if(canonicalResultTurnId!==undefined){const canonical=hiddenTurns.find(turn=>turn.turnId===canonicalResultTurnId);if(canonical===undefined)throw new Error('canonicalResultTurnId 未引用已知 hidden turn');if(canonical.state!=='completed')throw new Error('canonicalResultTurnId 必须引用 completed hidden turn')}
 if(['committed','cancelled','failed'].includes(status)){if(activeTurnId!==undefined)throw new Error(`终态 ${status} 不能保留 activeTurnId`);const pending=hiddenTurns.find(turn=>!['completed','failed','cancelled'].includes(turn.state));if(pending!==undefined)throw new Error(`终态 ${status} 不能包含非终态 hidden turn：${pending.turnId}`)}
 ...
 return{schemaVersion:1,transactionId,saveId,input:{channelId,text:inputText},inputFingerprint,baseProjectionRevision:Number(raw.baseProjectionRevision),status,hiddenTurns,operationRefs,...(activeTurnId===undefined?{}:{activeTurnId}),...(canonicalResultTurnId===undefined?{}:{canonicalResultTurnId}),...(diagnostic===undefined?{}:{diagnostic}),revision:Number(raw.revision),createdAt,updatedAt}
}
